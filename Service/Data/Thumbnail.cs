...
namespace net.vieapps.Services.Files
{
	...
	public class Thumbnail : Repository<Thumbnail>, IAttachment
	{
		...
		public string GetURI(string title = null, int width = 0, int height = 0)
		{
			...
			return $"{Utility.ThumbnailURI}{(string.IsNullOrWhiteSpace(this.SystemID) || !this.SystemID.IsValidUUID() ? this.ServiceName : this.SystemID)}/0/{width}/{height}/{this.ObjectID}/{index}/{this.LastModified:yyyy/fffssmmHHddMM}".ToLower() + (string.IsNullOrWhiteSpace(title) ? "" : $"/{title.GetANSIUri()}");
		}
